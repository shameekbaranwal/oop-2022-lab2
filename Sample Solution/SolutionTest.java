...
public class SolutionTest extends TestCase {
	...
	public SolutionTest(String name) {
		...
		commonCold = new Disease("Common cold", "Antibiotics", 50,
				new String[] { "Sneezing", "Runny nose", "Conestion", "Throat ache" });
	...
	/*
	 * The test cases. Note that the order of the test cases is as described in the problem statement.
	 */
	...
	public void testCase7() {
		...
		for(int i = 0; i < 3; ++i) {
			...
			assert(get[i].getCategory() == patients[i].getCategory());
		}
	...
	public void testCase10() {
		Disease hopefullyCommonCold = princeton.diagnose(four);
		Disease hopefullyCovid = princeton.diagnose(one);
		Disease hopefullySchizo = princeton.diagnose(three);
		
		assert (hopefullyCommonCold.getName() == commonCold.getName());
